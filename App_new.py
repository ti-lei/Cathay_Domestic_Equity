...
def dashboard():
	...
	Index = request.values.get('Index', 'TimeStemp')  #這裡如果get不到index 會給default值 'colors'
	Product = request.values.get('Product','IphoneXs')

	# df = get_df()
	df = pd.read_csv("Data.csv")

	#--------------------- Maping 不同的 國家的名字 ---------------------#
	All_countries = {'Us':'美國','Cn':'中國','Jp':'日本','Hk':'香港','Uk':'英國','De':'德國','Ru':'俄羅斯','Fr':'法國','Tw':'台灣',
						'Br':'巴西','Mx':'墨西哥'}
	All_countries_E = {'Us':'United State','Cn':'China','Jp':'Japan','Hk':'Hong kong','Uk':'Unkted Kingdom','De':'German','Ru':'Russia',
						'Fr':'French','Tw':'Taiwan','Br':'Brazil','Mx':'Mexico'}
	...
	pivot = pd.pivot_table(df, values='Deliver', index=Index,
	    columns=['All_countries','Colors','Size'], aggfunc=lambda x: ' '.join(x)).sort_index(ascending=False)

	#---------------------     協理要的國家排序    ---------------------#
	cols = ['美國','中國','香港','台灣','日本','德國','法國','俄羅斯','巴西','墨西哥']
	cols2 = ['美國','中國','香港','日本','德國','法國','俄羅斯']
	try:
		pivot = pivot[cols]
	except:
		pivot = pivot[cols2]
